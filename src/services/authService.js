...
const resetPassword = async (emailOrMobile, newPassword) => {
    // 1. Find user
    const findUser = (emailOrMobile) => new Promise((resolve, reject) => {
        User.findByEmailOrMobile(emailOrMobile, (err, rows) => {
            if (err) return reject(err);
            resolve(rows);
        });
    });
